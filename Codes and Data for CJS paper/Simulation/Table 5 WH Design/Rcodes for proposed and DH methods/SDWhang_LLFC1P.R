### 26 June 2022 Tabl3 5 WH Design
###
...
generate.Whang1 = function (L,nn1, nn2, SD) {  ## "xx" for F, "yy" for G
    temp = runif(nn1)
    yy = 0*(temp < 0.1)+(temp-0.1)*(temp<L & temp>=0.1) + (temp+0.1)*(temp>= L & temp < 0.9) + 1*(temp>=0.9)
    xx = runif(nn2)
...
ll = c(0.2,0.4,0.6,0.8)
Whang.pv = matrix(NA, length(ll), 12)

rownames(Whang.pv)=c("L=0.2","L=0.4","L=0.6","L=0.8")
...
write(c("Whang1; LLFC, TT0-3;nn1,nn2,Nrep, Boot.n",nn1,nn2,Nrep, Boot.n),"Whangpv1P",ncolumns=5,append=T)


write(round(t(Whang.pv),4), "Whangpv1P", ncolumns=12, append=T)

write.csv(Whang.pv,file="LLFC_Whangpv1P.csv")
